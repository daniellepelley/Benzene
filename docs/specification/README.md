...
| [versioning.md](versioning.md) | **Implemented, plus one proposed mechanism.** The three axes of version — handler, payload schema, and service — via the `benzene-version` header/route convention, handler-version dispatch vs. transparent payload casting (both shipped), and side-by-side service versions (§5, proposed: needs service version as a mesh identity layer) |
