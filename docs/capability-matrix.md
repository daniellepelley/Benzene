...
| **Resilience** | `UseRetry` — retry with exponential backoff, an optional max-delay cap, and pluggable jitter (`RetryMiddleware.FullJitter()`) | Circuit breaker, timeout, bulkhead — these are best served by a mature library (Polly), which Benzene will not hide behind its own abstraction | Drop your own Polly `ResiliencePipeline` into a ~15-line middleware step — see [Polly Resilience Pipelines](cookbooks/polly-resilience.md). Benzene gives Polly a clean place to plug in; it doesn't wrap it. See also [Resilience](resilience.md). |
