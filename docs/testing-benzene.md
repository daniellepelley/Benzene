...
## Testing a legacy `AwsLambdaStartUp`-based app

If your `StartUp` derives from the older, AWS-specific `AwsLambdaStartUp` (rather than the
platform-neutral `BenzeneStartUp` above), use `AwsLambdaBenzeneTestStartUp<TStartUp>` instead —
it predates `BenzeneTestHost` and is unrelated to it, but works the same way for this shape of
StartUp.
