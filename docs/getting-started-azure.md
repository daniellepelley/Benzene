...
The Azure Functions getting-started guide lives at **[Azure Functions Setup](azure-functions)** —
...
[Worker Service Setup, Part B](getting-started-worker#part-b-built-in-workers-kafka-http-service-bus-event-hub-cosmos-db)
(Service Bus, Event Hubs, and Cosmos DB Change Feed).

Related Azure cookbooks:

- [Managed Identity & RBAC](cookbooks/managed-identity) — run every Azure integration with no
  connection strings
- [Service Bus Message Handling](cookbooks/service-bus-handling)
- [Event Hub Stream Processing](cookbooks/event-hub-processing)
- [Cosmos DB Change Feed Processing](cookbooks/cosmos-change-feed-processing)

> This page exists so the family-consistent `getting-started-azure` URL resolves; the canonical
> guide is [`azure-functions`](azure-functions).
