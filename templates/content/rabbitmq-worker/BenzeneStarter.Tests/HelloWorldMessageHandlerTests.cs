...
// ConfigureServices and Configure run - then pushes a delivery through the whole Benzene pipeline.
// Only the RabbitMQ broker is simulated (no connection is opened). WithServices/WithConfiguration are
// the seams for overriding any dependency or setting the test needs (here we swap the real IGreeter
// for a spy so we can assert the handler actually ran with the routed message).
public class HelloWorldMessageHandlerTests
{
    ...
    public async Task Routing_the_demo_topic_invokes_the_handler()
    {
        ...
        await host.HandleAsync(
            MessageBuilder.Create("hello:world", new HelloWorldMessage { Name = "World" }).AsRabbitMqBenzeneMessage());
    ...
    public async Task An_unknown_topic_does_not_reach_the_handler()
    {
        ...
        await host.HandleAsync(
            MessageBuilder.Create("does:not-exist", new HelloWorldMessage { Name = "World" }).AsRabbitMqBenzeneMessage());
