...
- `BenzeneHealthCheckBridge` - an ASP.NET Core `IHealthCheck` that aggregates every registered
  `Benzene.HealthChecks.Core.IHealthCheck` (unhealthy if any failed, degraded if any warned, healthy
  otherwise); registered as the `"benzene"` check when `EnableHealthChecks` is set
